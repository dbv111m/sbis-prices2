...
### 3. Названия услуг и группы (nomenclature_map.json)

Технические коды номенклатуры (`EOpSBISfrmIoN`, `ETRN_50000`...) малопонятны,
поэтому отчеты используют гибридную схему:

1. **Скрапер** при сборе цен автоматически сохраняет название услуги с сайта
   (поле `name` в JSON);
2. **`nomenclature_map.json`** — редактируемый словарь расшифровок:
   человекочитаемое имя (`name`) и группу (`group`) для каждого кода, а также
   порядок групп (`group_order`). Mapping имеет приоритет над именем с сайта.

Коды, которых нет в mapping, получают имя с сайта и группу «Прочее» —
достаточно дополнить словарь.

Форматы отчетов:

- **CSV** — плоская таблица `Группа | Услуга | Код | регионы...`, сортировка по группам;
- **Excel** — лист с секциями-заголовками по группам, закрепленной шапкой и колонками `Услуга | Код | регионы...`.
...
- `nomenclature_map.json`: Расшифровка кодов номенклатуры — названия, группы и их порядок для отчетов.
